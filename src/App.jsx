...
HEAD
      ...
      // 🔥 THIS WAS MISSING
      if (key === "e") {
        if (nearObject === "newspaper") {
          setActivePuzzle("newspaper");
          setFeedback("");
        }

        if (nearObject === "portrait") {
  ...
  useEffect(() => {
    ...
    const nearNewspaper =
      Math.abs(playerPos.x - 150) < 80 && Math.abs(playerPos.y - 350) < 80;

    const nearPortrait =
      Math.abs(playerPos.x - 780) < 100 && Math.abs(playerPos.y - 250) < 120;

    if (room === 1 && nearNewspaper) {
      setNearObject("newspaper");
    } else if (room === 2 && nearPortrait) {
      setNearObject("portrait");
    } else {
      setNearObject(null);
    }
  }, [playerPos, room, view]);
